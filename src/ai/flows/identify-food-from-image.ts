...
const FoodItemSchema = z.object({
  ...
  nutrition: z.optional(z.any()).describe('Nutritional information for the identified food item.'),
});

const IdentifyFoodFromImageOutputSchema = z.object({
  foodItems: z.array(FoodItemSchema).describe('A list of identified food items with their portion sizes and nutritional information.'),
});
  ...
  output: {
    schema: z.object({
      foodItems: z.array(z.object({
        name: z.string().describe('The identified food item name.'),
        portionSize: z.string().describe('Estimated portion size of the food item (e.g., "1 cup", "1 slice", "100g").'),
      })).describe('A list of identified food items with estimated portion sizes.'),
    }),
  },
